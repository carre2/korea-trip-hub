...
export default function StayGuide({ city, cityKey }) {
  if (!city) return null;
        ...
        {city.areas.map((a) => (
          <div key={a.name} className={`stay-area stay-tone-${a.tone || "blue"}`}>
            ...
            <div className="stay-links">
              <a href={hotelsHref} target="_blank" rel="sponsored nofollow noopener">🏨 Find a hotel ↗</a>
              <a href={klookSearch(`${a.name} ${city.name}`)} target="_blank" rel="sponsored nofollow noopener">🎟️ Things to do ↗</a>
            </div>
          </div>
        ))}
      </div>
