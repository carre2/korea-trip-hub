...
/** Flatten markdown link syntax "[label](/path)" -> "label" for plain-text contexts
 *  like JSON-LD (structured data must not contain raw markdown). External URLs and
...
export function faqLd(items, locale) {
  ...
  return {
    ...
    mainEntity: qa.map(({ q, a }) => ({
      "@type": "Question",
      name: stripMd(q),
      acceptedAnswer: { "@type": "Answer", text: stripMd(a) },
    })),
  };
}
...
export function articleLd({ locale, path, headline, description, image, dateModified }) {
  const node = {
    ...
    publisher: {
      ...
      name: SITE_NAME,
      url: SITE,
      logo: { "@type": "ImageObject", url: `${SITE}${OG_IMAGE.url}`, width: OG_IMAGE.width, height: OG_IMAGE.height },
    },
  };
